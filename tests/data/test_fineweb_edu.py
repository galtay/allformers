...
@pytest.mark.slow
class TestFineWebEduSharding:
    """Tests for sharding FineWeb-Edu dataset across multiple workers (DDP support)."""

    def test_n_shards_property(self):
        """FineWeb-Edu default subset should have many shards for DDP."""
        ...
        # Default subset has 2410 shards
        assert n_shards > 100, f"Expected many shards, got {n_shards}"
        print(f"FineWeb-Edu default has {n_shards} shards")
    ...
    def test_shard_method_splits_data(self):
        ...
        # Collect first 2 document IDs from each rank
        worker_ids = []
